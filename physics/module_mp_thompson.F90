...
!> @{
      SUBROUTINE mp_gt_driver(qv, qc, qr, qi, qs, qg, ni, nr, nc,     &
                              ...
                              rainprod, evapprod,                     &
#endif
                              refl_10cm, sedisemi, sedi_semi_update,  &
...
!..Local variables
      ...
      !LOGICAL::sedisemi, sedi_semi_update
      REAL, PARAMETER :: decfl = 8.0
      !REAL, PARAMETER :: decfl = 2.0
      REAL :: dtcfl,precip
      INTEGER :: NITER
      ...
      enddo
        ...
        do n = 1, niter
          ...
          do k = 1,kte
            ...
            vtnrk_tmp(k) = vtnrk(k)
          enddo
          call nislfv_rain_plm(kte,rho,rhof,temp,dzq,vtrk_tmp,rr,precip,dtcfl,1,0,R1)                          
          call nislfv_rain_plm(kte,rho,rhof,temp,dzq,vtnrk_tmp,nr,vtr,dtcfl,1,0,R2)                                 
          do k = kts, kte
...
!-------------------------------------------------------------------
      SUBROUTINE nislfv_rain_plm(km,denl,denfacl,tkl,dzl,wwl,rql,precip,dt,id,iter,R1)
!         ,oams,a_,b_,Lam0,Lam1,fv_s,Kap0,Kap1,mu_s,csg,cse,av_s,T_0,vts_boost,ii,jj)
!      call nislfv_rain_plm(kte,rho,rhof,temp,dzq,vtsk,rs,rs,dt,1,1,&
...
!     print*,' ils1 ils2 mu_s t1-4 vts ',ils1,ils2,mu_s,t1_vts,t2_vts,t3_vts,t4_vts
!     print*,' k mrat lam0 lam1 kap0 kap1 boost ',k,mrat,lam0,lam1,kap0,kap1,vts_boost(k)
...
! rain out
      sum_precip: do k=1,km
                    if( za(k).lt.0.0 .and. za(k+1).le.0.0 ) then          !hmhj
                      precip = precip + qa(k)*dza(k)
                      cycle sum_precip
                    else if ( za(k).lt.0.0 .and. za(k+1).gt.0.0 ) then    !hmhj
